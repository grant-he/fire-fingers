...
class GameSettings {
    ...
    init?(data: [String: Any]) {
        guard let instantDeathModeEnabled = (data["instantDeathModeEnabled"] as? Bool) else {
            NSLog("GameSettings failed to convert instantDeathModeEnabled '\(String(describing: data["instantDeathModeEnabled"]))'")
            return nil
        }
        
        guard let earthQuakeModeEnabled = data["earthQuakeModeEnabled"] as? Bool else {
            NSLog("GameSettings failed to convert earthQuakeModeEnabled '\(String(describing: data["earthQuakeModeEnabled"]))'")
            return nil
        }
        
        guard let emojisAllowed = data["emojisAllowed"] as? Bool else {
            NSLog("GameSettings failed to convert emojisAllowed '\(String(describing: data["emojisAllowed"]))'")
            return nil
        }
        
        guard let playersCount = data["playersCount"] as? Int else {
            NSLog("GameSettings failed to convert playersCount '\(String(describing: data["playersCount"]))'")
            return nil
        }
